...
#ifdef BOARD_RA2L1_EK // BSP_MCU_GROUP_RA2L1
    // Master ready to send line
    // X6: Pin 6
    #define COMMS_MASTER_RTS_ACTIVE_VAL     (BSP_IO_LEVEL_LOW)
    #define COMMS_MASTER_RTS_INACTIVE_VAL   (BSP_IO_LEVEL_HIGH)
//    #define COMMS_MASTER_RTS_IS_ACTIVE()    (R_BSP_PinRead(ASL165_RTS) == COMMS_MASTER_RTS_ACTIVE_VAL)
//    #define COMMS_MASTER_RTS_INIT()         (R_BSP_PinCfg (ASL165_RTS, IOPORT_CFG_PORT_DIRECTION_INPUT))
    #define COMMS_MASTER_RTS_IS_ACTIVE()    (R_BSP_PinRead(HHP_RTS) == COMMS_MASTER_RTS_ACTIVE_VAL)
    #define COMMS_MASTER_RTS_INIT()         (R_BSP_PinCfg (HHP_RTS, IOPORT_CFG_PORT_DIRECTION_INPUT))
    #define COMMS_MASTER_RTS_DEINIT()

    // Slave ready to send/clear to send line
    // X6: Pin 2
    #define COMMS_RTS_CTS_ACTIVE_VAL        (BSP_IO_LEVEL_LOW)
    #define COMMS_RTS_CTS_INACTIVE_VAL      (BSP_IO_LEVEL_HIGH)
    #define COMMS_RTS_CTS_IS_ACTIVE()       (R_BSP_PinRead(ASL165_CTS) == COMMS_RTS_CTS_ACTIVE_VAL)
    #define COMMS_RTS_CTS_SET(active)       (R_BSP_PinWrite(ASL165_CTS, active ? COMMS_RTS_CTS_ACTIVE_VAL : COMMS_RTS_CTS_INACTIVE_VAL))
    #define COMMS_RTS_CTS_TOGGLE()          INLINE_EXPR(COMMS_RTS_CTS_SET(!COMMS_RTS_CTS_IS_ACTIVE()))
    #define COMMS_RTS_CTS_INIT()            R_BSP_PinCfg(ASL165_CTS, IOPORT_CFG_PORT_DIRECTION_OUTPUT)
    //#define COMMS_RTS_CTS_DEINIT()          INLINE_EXPR(TRISBbits.TRISB0 = GPIO_BIT_INPUT)

    // RX/TX data line
    // X6: Pin 3
    #define COMMS_DATA_ACTIVE_VAL           (BSP_IO_LEVEL_HIGH)
    #define COMMS_DATA_INACTIVE_VAL         (BSP_IO_LEVEL_LOW)
    #define COMMS_DATA_OUT_IS_HIGH()        (R_BSP_PinRead(ASL165_DATA) == BSP_IO_LEVEL_HIGH)
    #define COMMS_DATA_IN_IS_HIGH()         (R_BSP_PinRead(ASL165_DATA) == BSP_IO_LEVEL_HIGH)
    #define COMMS_DATA_SET(active)          R_BSP_PinWrite(ASL165_DATA, active ? COMMS_DATA_ACTIVE_VAL : COMMS_DATA_INACTIVE_VAL)
    #define COMMS_DATA_TOGGLE()             (COMMS_DATA_SET(!COMMS_DATA_IS_ACTIVE()))
    #define COMMS_DATA_CONFIG_RX()          R_BSP_PinCfg(ASL165_DATA, IOPORT_CFG_PORT_DIRECTION_INPUT | IOPORT_CFG_PULLUP_ENABLE)
    #define COMMS_DATA_CONFIG_TX()          R_BSP_PinCfg(ASL165_DATA, IOPORT_CFG_PORT_DIRECTION_OUTPUT)

    // Clock line
    // X6: Pin 5
    #define COMMS_CLK_ACTIVE_VAL            (BSP_IO_LEVEL_HIGH)
    #define COMMS_CLK_INACTIVE_VAL          (BSP_IO_LEVEL_LOW)
    #define COMMS_CLK_OUT_IS_HIGH()         (R_BSP_PinRead (ASL165_CLK) == BSP_IO_LEVEL_HIGH)
    #define COMMS_CLK_IN_IS_HIGH()          (R_BSP_PinRead (ASL165_CLK) == BSP_IO_LEVEL_HIGH)
    #define COMMS_CLK_SET(active)           R_BSP_PinWrite (ASL165_CLK, active ? COMMS_CLK_ACTIVE_VAL : COMMS_CLK_INACTIVE_VAL)
    #define COMMS_CLK_TOGGLE()              (COMMS_CLK_SET(!COMMS_CLK_IS_ACTIVE()))
    //#define COMMS_CLK_CONFIG_RX()           R_BSP_PinCfg(ASL165_CLK, IOPORT_CFG_PORT_DIRECTION_INPUT)
    #define COMMS_CLK_CONFIG_RX()           R_BSP_PinCfg(ASL165_CLK, IOPORT_CFG_PORT_DIRECTION_INPUT | IOPORT_CFG_PULLUP_ENABLE)
    #define COMMS_CLK_CONFIG_TX()           R_BSP_PinCfg(ASL165_CLK, IOPORT_CFG_PORT_DIRECTION_OUTPUT)

#elif (defined _18F46K40)
    // Master ready to send line
    // X6: Pin 6
...
{
    if (WaitForMstRtsPinToGoInactive(0xff)) // HA_HHP_LINE_INPUT_TO_100_us))
	{
		StartPacketTransmit();
...
bool haHhpBsp_RxByte(uint8_t *rxd_byte)
{
	...
	*rxd_byte = 0;
	...
	while ((num_bits_read_in < 8) && ret_val)
	{
		...
		ret_val = WaitForClkPinToGoHigh (0xff); // (HA_HHP_LINE_INPUT_TO_100_us);

		if (ret_val)
		{
			// Wait for the clock line to go low.
			ret_val = WaitForClkPinToGoLow (0xff); // (HA_HHP_LINE_INPUT_TO_100_us);
...
void haHhpBsp_SlaveReadyToReceivePacket(void)
{
    ...
    //vTaskDelay (1); // 1 msec is too long
	for (int i = 0; i < 100; ++i)
	    ;
	COMMS_RTS_CTS_SET(false);
}
...
static void StartPacketTransmit(void)
{
    ...
    //vTaskDelay (1); // 1 msec may be too long
    for (int i = 0; i < 200; ++i)
        ;
}
...
static void TxByte(uint8_t tx_byte)
{
	for (unsigned int i = 0; i < 8; i++)
	{
		...
		//bspDelayUs(US_DELAY_50_us);
	    // vTaskDelay (1); // 1 msec may be too long
	    for (int myTimeLoop = 0; myTimeLoop < 200; ++myTimeLoop)
	        ;
		COMMS_CLK_SET(false);
